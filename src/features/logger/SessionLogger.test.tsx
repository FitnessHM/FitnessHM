...
beforeEach(async () => {
  await Promise.all([db.blocks.clear(), db.sessions.clear(), db.logs.clear(), db.efforts.clear()]);
});
...
test('logging a time trial with distance and duration records a checkpoint effort', async () => {
  const block = await createBlock({
    eventDistanceMeters: 5000, eventLabel: '5K', goalType: 'time', goalTimeSeconds: 1200,
    targetDate: '2026-11-01', baselineSource: 'known', raceMantra: null, targetSplitSecondsPerKm: null,
  });
  const session = await createPrescribedSession({
    blockId: block.id, date: '2026-09-01', type: 'time_trial', reps: null,
    targetPaceSecondsPerKm: null, targetRestSeconds: null, targetDurationSeconds: null, note: null,
  });
  ...
  expect(efforts[0].kind).toBe('checkpoint');
  expect(efforts[0].distanceMeters).toBe(5000);
  expect(efforts[0].timeSeconds).toBe(1470);
});

test('logging a non-time-trial session records no effort', async () => {
  const block = await createBlock({
    eventDistanceMeters: 5000, eventLabel: '5K', goalType: 'time', goalTimeSeconds: 1200,
    targetDate: '2026-11-01', baselineSource: 'known', raceMantra: null, targetSplitSecondsPerKm: null,
  });
  const session = await createPrescribedSession({
    blockId: block.id, date: '2026-09-01', type: 'tempo', reps: null,
    targetPaceSecondsPerKm: null, targetRestSeconds: null, targetDurationSeconds: 1200, note: null,
  });

  const user = userEvent.setup();
  render(<SessionLogger blockId={block.id} session={session} onSaved={vi.fn()} />);

  fireEvent.change(screen.getByLabelText(/duration/i), { target: { value: '1200' } });
  fireEvent.change(screen.getByLabelText(/distance/i), { target: { value: '4000' } });
  await user.click(screen.getByRole('button', { name: /save/i }));
